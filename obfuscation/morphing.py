...
def _rename_all_identifiers(source):
    """Rename every function and module-level variable to a unique random name.

    This is the most impactful transformation for defeating byte-level
    similarity detection — every identifier in every build is different.
    """
    builtins_skip = {
        'print', 'len', 'range', 'str', 'int', 'float', 'bool', 'list',
        'dict', 'set', 'tuple', 'bytes', 'bytearray', 'type', 'isinstance',
        ...
        '__enter__', '__exit__', '__str__', '__repr__', '__call__',
        '__getitem__', '__setitem__', '__delitem__', '__len__', '__iter__',
        '__next__', '__contains__',
        'main', 'args', 'kwargs',
    }

    func_names = re.findall(r'^(?:async )?def (\w+)\s*\(', source, re.MULTILINE)
    var_names = re.findall(r'^(\w+)\s*=\s', source, re.MULTILINE)

    candidates = set()
    for name in func_names + var_names:
        if (name not in builtins_skip
                and not name.startswith('__')
                and len(name) > 1):
            candidates.add(name)

    name_map = {}
    styles = ['hash', 'word', 'underscore']
    style = random.choice(styles)
    word_pool = [
        'signal', 'matrix', 'delta', 'vector', 'stream', 'kernel',
        'buffer', 'cache', 'queue', 'stack', 'frame', 'token',
        'cipher', 'digest', 'block', 'index', 'table', 'relay',
        'pulse', 'node', 'edge', 'path', 'link', 'gate', 'port',
        'core', 'sync', 'pipe', 'mesh', 'flux', 'zone', 'grid',
    ]
    ...
    for name in sorted(candidates):
        if style == 'hash':
            ...
            w1, w2 = random.sample(word_pool, 2)
            name_map[name] = f"{w1}_{w2}_{random.randint(10,99)}"
        else:
            name_map[name] = f"_proc_{random.randint(1000,9999)}_{random.randint(10,99)}"
...
def _encode_all_strings(source):
    """Encode every string literal with a random encoding scheme.

    Each string gets a different encoding on each build, breaking
    byte-level fingerprinting of string constants.
    """
    lines = source.split('\n')
    result = []
    ...
    for line in lines:
        ...
        def _replace_str(m):
            ...
            if random.random() < 0.35:
                return m.group(0)
            return atomize_string(s)

        line = re.sub(r"'[^']{3,80}'", _replace_str, line)
        line = re.sub(r'"[^"]{3,80}"', _replace_str, line)
        result.append(line)
...
def morph_python_source(source, intensity='high'):
    ...
    3. Rename all identifiers (most impactful for uniqueness)
    4. Encode all string literals
    5. Add variable junk padding
    6. Numeric constant mutation
    7. Anti-debug injection
    8. Line-level mutations (opaque predicates, dead code, expression mutation)
    """
    # Phase 1: Structural transformations
    if intensity == 'high':
        source = _shuffle_functions(source)
        source = _randomize_imports(source)
        source = _rename_all_identifiers(source)
        source = _encode_all_strings(source)
