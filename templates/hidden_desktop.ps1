...
namespace Win32 {
  public static class Hd {
    ...
    public static string Go(string command) {
      ...
      if (action == "type") return Type(arg);
      if (action == "key") return Key(arg.Length == 0 ? 13 : int.Parse(arg));
      if (action == "stop") { if (desk != IntPtr.Zero) { CloseDesktop(desk); desk = IntPtr.Zero; } return "desktop stopped"; }
    ...
    static string Type(string text) {
      ...
      foreach (var ch in text) {
        ...
        short pair = VkKeyScanW(ch); byte vk = (byte)(pair & 0xFF); if (vk == 0xFF) continue;
    ...
    static string Key(int vk) {
      if (!Ensure() || !SetThreadDesktop(desk)) return "desktop open failed";
      keybd_event((byte)vk, 0, 0, 0); keybd_event((byte)vk, 0, 2, 0);
      return "key " + vk;
    }
    ...
    [StructLayout(LayoutKind.Sequential)]
    struct BITMAPINFO { public int biSize, biWidth, biHeight; public short biPlanes, biBitCount; public int biCompression, biSizeImage, biXPelsPerMeter, biYPelsPerMeter, biClrUsed, biClrImportant; }
